...
?>
     ...
     <?php
     $db= new PDO ("mysql:host=localhost;dbname=spk_ahp_v1", "root", "");
 	//$servername="localhost";
   // $username="id2333746_root";
   // $password="lukman1805";
...
/*try {
  ...
  // }
   $hasiltung=0;
   $query="select bobot_kriteria,jumlah_kriteria from ahp_data_kriteria where id_pengguna = '$_SESSION[id_pengguna]'";
	$results = $db->query($query);
  // $a1="select bobot_kriteria,jumlah_kriteria from ahp_data_kriteria where id_pengguna = '$_SESSION[id_pengguna]'";
   // $hasil=mysqli_query($a1);
    //   if($hasil===FALSE){
      //  die(mysqli_error());
     //  }
     ...
     while($row = $results->fetch(PDO::FETCH_ASSOC)){
        ...
        $ab = $row['bobot_kriteria'] ; 
        $ac = $row['jumlah_kriteria'] ;
        $hitung= $ab*$ac;
        $hasiltung = $hitung+ $hasiltung;
         $mn= $hasiltung-5;
  ...
  echo "<center>";
  echo "<strong>";
  echo "<font size='3'>Consistency Ratio adalah <font color='red'> $xm </font></font>";
  //echo "<br>";
  if($xm < 0.1){
   // echo "<button type='button' class='btn btn-success btn-lg'>Terpenuhi</button>";
    echo "<div class='alert alert-success'>
  <strong>Terpenuhi!</strong> Consistency Ratio Terpenuhi Silahkan Lanjutkan ke Analisis Alternatif.
</div>";

  }else
  {
   //echo " <font size='5' color='red'>Tidak terpenuhi</font><br>";
    echo "<div class='alert alert-danger'>
  <strong>Tidak Terpenuhi!</strong> Consistency Ratio Tidak terpenuhi Silahkan Masukan Kembali Nilai Perbandingan Berpasangan Kriteria</div>";
  }
  echo "</strong>";
  echo "</center>";
   ...
   ?>
   <center>
      <font size='3'> <p>Jika consistency ratio tidak terpenuhi silahkan masukan ulang analisa kriteria anda hingga <b>Consistency Ratio</b> Terpenuhi. untuk dapat terpenuhi harus bernilai < 0,1 </p></font>
   </center>
   <div class="col-md-12 text-center">
    <button type="button" onclick="location.href='analisa-kriteria.php'" class="btn btn-primary"><span class="fa fa-clone"></span> Analisis Kriteria Kembali</button>
    
            <button type="submit" onclick="location.href='analisa-alternatif.php'" name="hapus-contengan" class="btn btn-success"><span class="fa fa-balance-scale"></span>Lanjut Ke Analisis Alternatif Jurusan</button>
    </div>
</div>
</div>			
<?php
